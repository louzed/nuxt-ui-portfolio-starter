      ...
      <nav class="flex items-center justify-start">
        <ul class="flex gap-2">
          <li v-for="link in navLinks" :key="link.id">
            <ClientOnly>
              <AppHashes :name="link.name" :section-id="link.id" />
            </ClientOnly>
          </li>
        </ul>
      </nav>
...
import { useLocalePath } from '#imports'; // Import useLocalePath


const { t } = useI18n();
const localePath = useLocalePath(); // Initialize useLocalePath
